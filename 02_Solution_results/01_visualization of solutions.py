...
for i, solution_file in enumerate(solution_files):
    # 打开DEM文件
    data_dem = rs.open(solution_file)

    # 在子图中显示DEM图像
    row = i // 5  # 行索引
    col = i % 5  # 列索引

    show(data_dem, title=f'DEM_solution_{i + 1}', ax=axes[row, col])
    axes[row, col].set_title(solution_file)
...
plt.savefig("All DEM_solution figure")
plt.show()
...
for i, dem_file in enumerate(dem_files):
    ...
    show(data_dem, title=f'DEM_after_{i + 1}', ax=axes[row, col])
